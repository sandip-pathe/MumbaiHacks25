...
import { RepoSelection } from "./views/RepoSelection";
...
const App: React.FC = () => {
  ...
  const handleAuthNavigate = (
    target: "LANDING" | "LOGIN" | "REGISTER" | "ONBOARDING"
  ) => {
    switch (target) {
      case "LANDING":
        setView("AUTH_LANDING");
  ...
  if (view === "REPO_SELECTION") {
    return (
      <RepoSelection
        githubToken={githubToken}
        onComplete={handleRepoSelectionComplete}
      />
    );
  }
    ...
    <div className="min-h-screen flex bg-bgMain text-white selection:bg-primary/30 animate-fade-in">
        ...
        <div className="p-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-accent3 uppercase tracking-wider">
                AI Compliance
              </p>
            </div>
          </div>
      ...
      <main className="flex-1 md:ml-64 min-h-screen relative">
        ...
        <header className="sticky top-0 z-40 bg-bgMain/80 backdrop-blur-md border-b border-white/5 h-16 flex items-center justify-between px-6 md:px-10">
          <div className="md:hidden font-heading font-bold text-xl">
            ReguPulse
          </div>
        ...
        <div className="p-6 md:p-10 max-w-7xl mx-auto">
          ...
          {view === "RUN_DETAIL" && selectedRunId && (
            <RunDetail runId={selectedRunId} onBack={handleBackToDashboard} />
          )}
...
const NavItem: React.FC<{
  ...
  >
    <span
      className={`${
        active ? "text-primary" : "text-accent3 group-hover:text-white"
      }`}
    >
      {icon}
    </span>
